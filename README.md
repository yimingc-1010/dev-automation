...
- `docs/superpowers/specs/assets/` — spec 內 3 段 Mermaid 圖 + DR 決策樹的預渲染 SVG + PNG(用 `mermaid-config.json` + `puppeteer-config.json` 指定 Heiti TC 字型)
- `docs/disaster-recovery.md` — **Phase 0 完成**。涵蓋 4 個情境:中央 repo 失能 / Secret rotate / Org 變 Personal / Bot 服務掛了,含 Mermaid 決策樹。
...
```
Phase 0  ✅ 災難復原文件 (退場機制) — docs/disaster-recovery.md
Phase 1  ⏳ home-splat 單 repo 驗證(下一個動作)
Phase 2  ⏳ 抽出 reusable workflow 進此 repo
Phase 3  .github repo + Starter Workflows 推廣(Phase 2 完成後)
```
